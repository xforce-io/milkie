/**
 * Record a sample run for the s-002 inspect example.
 *
 *   $ npx tsx examples/s-002-inspect/record.ts
 *
 * Writes events to `.milkie/runs/<runId>.jsonl` and the chosen runId to
 * `.milkie/last-run.txt` so `report.sh` can default to it. The agent here
 * uses a tiny in-process StubGateway — no API key required.
 *
