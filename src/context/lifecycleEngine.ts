...
import type { Message, MessageContent, JSONValue, ContextProjection } from '../types/common.js'
...
// #192: render delivered projections as a labeled background block (plain text).
// Shared by the current-turn region (where projections are now merged) and by
// makeExternalContextRegion (kept for snapshot rehydration of legacy regions).
export function renderDeliveredContextBlock(projections: ContextProjection[]): string {
  const blocks = projections.map((p, index) => {
    const lines = [
      `#${index + 1}`,
      `producedBy.runId: ${p.sourceRunId}`,
      ...(p.sourceContextId ? [`producedBy.contextId: ${p.sourceContextId}`] : []),
  ...
  return [
    '--- External Delivered Context ---',
    'These are reports or outputs already delivered to the user by external runs.',
    'They are not a user message and not history generated by this context.',
    ...blocks,
  ].join('\n')
}
...
export function makeExternalContextRegion(projections: ContextProjection[]): RegionInput {
  return {
    ...
    format:    (c): Message => ({
      role:    'user',
      content: [{ type: 'text', text: renderDeliveredContextBlock(c as ContextProjection[]) }],
    }),
  }
}
...
interface CurrentTurnContent {
  input:       string
  projections: ContextProjection[]
}

// #192: extract the user's real input from a current-turn region's content.
// Tolerates the legacy string shape so old snapshots rehydrate cleanly.
export function currentTurnInput(content: unknown): string {
  if (typeof content === 'string') return content
  return (content as CurrentTurnContent).input
}

export function makeCurrentTurnRegion(
  input: string,
  projections: ContextProjection[] = [],
): RegionInput {
  return {
...
export function rehydrateRegion(r: Region): Region {
  ...
  if (r.id === 'current-turn') {
    return { ...r, format: makeCurrentTurnRegion('').format }
  }
  ...
  if (r.id === 'external-context') {
    return { ...r, format: makeExternalContextRegion(r.content as ContextProjection[]).format }
  }
