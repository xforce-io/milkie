...
### 4.1 `milkie agent run <agentId>` — 执行 agent

| Arg / Flag | 说明 |
|---|---|
| `<agentId>` | 位置参数，已注册的 agent id |
...
**注册前提.** `<agentId>` 必须是已注册的 agent。CLI 启动时从
`.milkie/agents.json` manifest auto-load；详见
`docs/superpowers/specs/2026-05-24-agent-registration-design.md`。SDK
consumer 仍可通过 `Milkie.registerAgent()` / `loadAgentFile()` 程序化注册。
`milkie agent list` 暴露 registry 的快照。
...
**Output.** JSONL：每行 `{ agentId, description?, registeredAt?, metadata? }`。
...
| 3 | Not found（runId / eventId / suiteId / contextId / agentId 不存在） |
...
- `runId`、`eventId`、`suiteId`、`contextId`、`agentId` 都是 opaque string，caller 不应解析
- `agentId` 由 registry 命名空间约束（人可读，但 caller 应当 treat as opaque）
...
| CLI | SDK |
|---|---|
| `milkie agent run <agentId> --input <text>` | `Milkie.invoke({ agentId, input, contextId? })` |
...
- ~~**Agent registration mechanism.**~~ ✓ 已 closed by
  `docs/superpowers/specs/2026-05-24-agent-registration-design.md`：
  `.milkie/agents.json` manifest convention，CLI 启动时 eager auto-load，
  SDK consumer 可选用 `Milkie.loadManifest()` 或继续手写
  `registerAgent()` / `loadAgentFile()`。
