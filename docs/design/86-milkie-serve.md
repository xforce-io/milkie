...
| 起止 / 终态 | `agent.run.started` / `agent.run.completed` | 持久化广播 |
...
- **持久化事件**:`BroadcastingEventStore`(复用 agent-docs-qa 模式)按 `contextId` 广播(靠 `agent.run.started` 建 runId→contextId 映射,`subscribe(contextId, cb)`);`POST /chat` handler 订阅该 contextId。
- **`message_delta`**:`POST /chat` 调 `invoke` 时传入 `onModelEvent`,在 handler 闭包内(已知 contextId)把 delta 写入同一条 SSE 响应。
...
2. **明确的终态事件** — 流必须以一个带 `status` + `output` 的终态事件收尾,alfred 靠它判定本轮结束并取最终结果(也是「省 /status」成立的前提)。
   - 落地:`AgentRunCompletedPayload` **自带** `status: 'completed' | 'interrupted' | 'error'`、`lastTextOutput?`、`error?`。直接广播 `agent.run.completed` 即为终态事件,SSE 层发完该事件后关闭流。无需额外合成。
...
4. **error 不静默** — LLM/工具出错时,以 `error` 事件 + 终态事件出现在流里,**不静默断开连接**(否则 alfred 无法区分「出错」与「网络断」)。
   - 落地:`POST /chat` 内 `invoke` reject 时,SSE 层 catch → 写一个 `error` 事件(`{code,message}`)+ `agent.run.completed{status:'error',error}` 终态 → 关闭流。
